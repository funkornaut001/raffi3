    ...
    AutomationCompatibleInterface 
{
 ...
 /**
    ...
    function performUpkeep(
        ...
        require(raffle.status == STATUS.ACCEPTED, "Raffle in wrong status. NFT was not staked");
    ...
    function createRaffle(
        ...
        require(isTokenApproved(_paymentInfo.paymentToken), "ERC20 payment token not approved");
        }
    ...
    */
    function transferNFTAndFunds(
        uint256 _raffleId
    ) external nonReentrant onlyTrustedCaller(_raffleId) { //TODO investigate this internal vs external function, will internal work better if transferNFT is in called in fulfillRandomWords?
        RaffleStruct storage raffle = raffles[_raffleId];
        ...
        uint256 amountForSeller = raffle.paymentInfo.amountRaised - amountForPlatform;
         
        
        if (raffle.paymentInfo.isNativeToken == false) {
        // transfer 95% to the seller
        IERC20(raffle.paymentInfo.paymentToken).transfer(raffle.seller, amountForSeller);
        // transfer 5% to the platform
        IERC20(raffle.paymentInfo.paymentToken).transfer(destinationWallet, amountForPlatform);
        } else {
    ...
    function getBalance(uint256 _raffleId) public view returns (uint256) {
        RaffleStruct storage raffle = raffles[_raffleId];
        if (raffle.paymentInfo.isNativeToken == false) {
            return IERC20(raffle.paymentInfo.paymentToken).balanceOf(address(this));
        } else {
    ...
    function extractFunds(uint256 _raffleId) public payable onlyOwner {
        ...
        payable(owner()).transfer(raffle.paymentInfo.amountRaised);
            ...
            IERC20(raffle.paymentInfo.paymentToken).transfer(owner(), raffle.paymentInfo.amountRaised);
        }
    }
    ...
    */
    ...
    //     if (raffle.paymentInfo.amountRaised == 0) {
