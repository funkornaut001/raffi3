...
// /*
// Notes:
// took out set Winner function
// made raffle ctrator = msg.sender in createRaffle
// Changes solidity version to most current
// commented out //require(raffle.entriesLength > 0, "Raffle has no entries"); in performUpkeep

// 4/27

// 4/23 THIS IS NOW THE LATEST VERSION!!!!
//  will keep testing 0x38E2Fe2A0b0859DD713e49991f24A80bbac91d2e on remix.
// ---0x82392c5FE43a7183f1E3153f307d80BCe8cFdF6a 0x3f owner

// Tried to get rid of price struct but to do so I needed to add priceperticketinwei to the raffle struct whcih gives stack too deep


// 4/15 MyToken 0x4d0976362161e8b0a8e8092d4c54cb3dBc76749D added to list, added 0x0000000000000000000000000000000000001010 (maybe Matic token erc20)
// - no need to enter raffle creator w/ createRaffle
// - getRaffle needs to return paymentTOken
// -- will we need chianlink pricefeeds to get the correct price of tickets if they are bought in WETH?
// -- not sure how to accept raffles w/ native MATIC token now
// -- is pricePerWeis the way to go? We will have to do the math on how many Weis = ERC20 token 
// - raffle 0 worked w/ normal flow. Tickets bought w/ MyToken, sold out, Automation and VRF worked, transferNFTAndFunds worked properly with ERC20 MyToken

// 4/15 changes
// removed raffle creator from createRafffle parameters, set it as address _raffleCreator = msg.sender in the body of function

// 4/16 added bool isNativeToken to paymentInfo. Used where transfering tokens happens with if(isNative){}else{}
// getRaffle() now returns the bool isnativetoken & price per ticket, took out platform fee as that should be constant, if any more params are added here we'll get stack to deep
// */
...
//         bool isERC1155; // whether the NFT is ERC1155 or not
...
//         uint256[] randomWords;
//     }
//     // TODO - do we need this pricing structure?
//     struct PriceStructure {
//         uint256 id;
//         uint256 numEntries;
//         uint256 price;
//     }
//     // TODO - do we need this funding structure? just say in create raffle ticket price must be above 1 wei
...
//     // TODO - do we need this funding structure?
...
//     // mapping of address to raffleId to get raffleId for onERC1155Received b/c stakeNFT is an internal function
//     mapping(address => uint256) private expectedRaffleId;
...
//     // this function is called buy chainlink automation performUpkeep(). It will request a random number from the VRF
...
//             upkeepNeeded = (isAccepted && isReady);
//             if (upkeepNeeded) {
...
//             "Raffle in wrong status. NFT was not staked"
//         );
...
//             price: _paymentInfo.pricePerTicketInWeis
//         });
...
//         if (raffle.isERC1155 == true) {
//             IERC1155 token = IERC1155(raffle.collateralAddress);
//             require(
//                 token.balanceOf(msg.sender, raffle.collateralId) > 0,
//                 "NFT is not owned by caller"
//             );
//             // TODO should the transferhappen the status change?
//             raffle.status = STATUS.ACCEPTED;
//             expectedRaffleId[msg.sender] = _raffleId; // Set the expected raffle ID for the user
...
//         require(
//             raffle.status == STATUS.ACCEPTED,
//             "Raffle is not in accepted. AKA the NFT was never staked / sent to the contract" //TODO change error message to just 1st sentence
//         );
...
//         uint256 totalDue = (_numberOfTickets * priceStruct.price);
...
//             );
//             paymentToken.transferFrom(msg.sender, address(this), totalDue);
...
//             IERC20(raffle.paymentInfo.paymentToken).transfer(
//                 raffle.seller,
//                 amountForSeller
//             );
...
//     uint256 _raffleId = expectedRaffleId[_from];
//     RaffleStruct storage raffle = raffles[_raffleId];

//     // Make sure the raffle is in the ACCEPTED state
//     require(raffle.status == STATUS.ACCEPTED, "Raffle not accepted");

//     // Make sure the transferred token matches the raffle collateral
//     require(_id == raffle.collateralId, "Invalid token ID");

//     // Clear the expected raffle ID for the sender
//     expectedRaffleId[_from] = 0;

//     // Return the ERC1155 receiver selector
...
// // // Decode _raffleId from _data parameter
// //     uint256 _raffleId = expectedRaffleId[_from];

// //     RaffleStruct storage raffle = raffles[_raffleId];
// //     _operator = msg.sender;
// //     _from = msg.sender;
// //     _id = raffle.collateralId;
// //     _value = 1;
// //     _data = "";
...
// //   function supportsInterface(bytes4 interfaceID) public view override (bool) {
...
//             // raffle.paymentInfo.paymentToken,
...
//         if (raffle.paymentInfo.isNativeToken == true) {
//             // transfer to the contract owner
...
//     // TODO: add a check to make sure the raffle randomnumbe aviailable is false
